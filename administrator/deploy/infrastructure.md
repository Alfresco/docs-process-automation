...
Integration events are events that are emitted by [connectors](../../modeling/connectors/index.md). For example, if using the [email connector](../../modeling/connectors/email.md), the event bridge will listen to events in the queue named `emailConnector.SEND`. 

### Elasticsearch

The following are the key value pairs that can be set when using [Elasticsearch](https://www.elastic.co/){:target="_blank"}:
...
The following are the key value pairs that can be set when using [Kafka](https://kafka.apache.org/){:target="_blank"}:
