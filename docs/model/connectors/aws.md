...
The **INVOKE** action is used by the Lambda connector to invoke [Amazon Web Services (AWS) Lambda functions](https://aws.amazon.com/lambda/).
...
The possible [errors](README.md#errors) that can be handled by the Lambda connector are:
...
The **ENTITY** action is used by the Comprehend connector to execute [Amazon Comprehend](https://aws.amazon.com/comprehend/) natural language processing (NLP) services and identify and analyze text from `UTF-8` plain text files.
...
| Parameter | Type | Description |
| --------- | ---- | ----------- |
| file | File | *Requires one.* A [variable](../processes/README.md#process-variables) of type file to send for analysis. |
...
| aisResponse | JSON | *Optional.* The result of the analysis in [Alfresco Intelligence Service](https://docs.alfresco.com/intelligence/concepts/ai-welcome.html) format. |
...
The possible [errors](README.md#errors) that can be handled by the Comprehend connector are:
...
The **LABEL** action is used by the Rekognition connector to execute [Amazon Rekognition](https://aws.amazon.com/rekognition/) services to identify and label the objects in JPEG and PNG files that are less than 15mb in size.
...
Files between 5mb and 15mb are uploaded to an Amazon S3 bucket before processing. The [IAM](https://aws.amazon.com/iam/) user configured to run the Rekognition service requires access to this bucket, the [Rekognition service itself](https://docs.aws.amazon.com/rekognition/latest/dg/access-control-overview.html) and to have the `rekognition:DetectLabels` permission.

The input parameters of the Rekognition connector are:

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| file | File | *Required.* A [variable](../processes/README.md#process-variables) of type file to send for analysis. |
| mediaType | String | *Optional.* The media type of the file to be analyzed, for example `application/octect-stream`. |
...
| aisResponse | JSON | *Optional.* The result of the image analysis in [Alfresco Intelligence Service](https://docs.alfresco.com/intelligence/concepts/ai-welcome.html) format. |
...
The possible [errors](README.md#errors) that can be handled by the Rekognition connector are:
...
The **EXTRACT** action is used by the Textract connector to execute [Amazon Textract](https://aws.amazon.com/textract/) to extract text and metadata from JPEG and PNG files that are less than 5mb in size.
...
The [IAM](https://aws.amazon.com/iam/) user configured to run the Textract services needs to have the `textract:DetectDocumentText` and `textract:AnalyzeDocument` permissions.

The input parameters of the Textract connector are:

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| file | File | *Required.* A [variable](../processes/README.md#process-variables) of type file to send for extraction. |
...
The possible [errors](README.md#errors) that can be handled by the Textract connector are:
