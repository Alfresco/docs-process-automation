...
* [Events](#event) that are used in conjunction with a [trigger](triggers.md) to start a trigger action when clicked by a user.
* [Forms](#form) that display a [form](forms.md) to the user to fill in, independent of a user task or process.
...
Event actions are directly referenced by [triggers](triggers.md) when they are clicked. This means that an action can post a predefined message to a Slack channel or send an email notifying an email group about the document.

**Note:** If a [user interface trigger action](triggers.md#user-interface) is not created that references the event action then nothing will happen when the action is clicked by an end user.
...
Form actions open a [form](forms.md) in Digital Workspace that users can fill in when the action is clicked. The form in question can be completely independent of a process and user task. Forms allow users to enter some information and use the output, for example to use a [trigger](triggers.md#forms) on the form submission event.
...
A start process action will start a process when the action is clicked. If the selected process contains a form on the [start event](processes/bpmn.md#start-events) with an [attach file field](forms.md#attach-file-fields) and `Use selected nodes in action` is set to `true`, then the file that was selected when the action was clicked will be attached to it automatically.
...
![application header image](../images/feature-app-header.png)
...
![application header menu image](../images/feature-app-header-menu.png)
...
![viewer toolbar menu image](../images/feature-viewer-toolbar-menu.png)

#### Viewer shared feature

The shared viewer will display an action on the file viewer page of a shared file:

![viewer shared image](../images/feature-viewer-shared.png)
...
![viewer shared toolbar image](../images/feature-viewer-shared-toolbar.png)
